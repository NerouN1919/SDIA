...
| PaymentFailed Transaction Query Service -> | Kafka | event | Да | Без DLQ(payment.wallet.result.dlq) read модель не будет поддерживать eventual consitnency(возможная ошибка парсинга или недоступность API ElsaticSearch) |
...
При большой нагрузке в Kafka нет как такового механизма backpressure, вместо этого сообщения как обычно сохраняются в журнал кафки, а у консьюмеров растетт оффсет - консьюмер лаг. При увеличении нагрузки - требуется горизонтально масштабировать консьюмеров и увеличивать число партиций топика. Консьюмеров должно быть столько же, сколько партиций в топике.  
